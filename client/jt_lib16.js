function JT(id,w,h,fps,setupName,updateName,objName,mobileAudioSize,fullScreenBtn){
    ...
    this.canvas={
        ...
        resize:function(w,h,camReset){
			...
			if(this.context.draw.canvas!=undefined){
				this.context.draw.canvas.w=w;
				this.context.draw.canvas.h=h;
				
				
				if(camReset=undefined){
					this.context.draw.camReset();
				}else{
					if(camReset==false){
						//nothing
					}else{
			...
			this.context.draw.alpha(alpha);
			this.context.draw.align(align);
			this.context.draw.baseline(baseLine);
        },
		//Get ratio
		ratio:function(){
        ...
        mainLoop:function(){
            ...
            if(this.loaded==false){
              ...
              for (var im in this.context.assets.images) {
                ...
                //cam
                if(this.context.draw.cam.w<0.001){
                    this.context.draw.cam.w=0.001;
                }
                
                if(this.context.draw.cam.h<0.001){
                    this.context.draw.cam.h=0.001;
                }
				...
				if(this.context.draw.cam.active){
					//mouse
					var mX=this.context.mouse.x;
					var mY=this.context.mouse.y;
					
					var camW=Math.abs(this.context.canvas.src.width/this.context.draw.cam.w)
					var camH=Math.abs(this.context.canvas.src.height/this.context.draw.cam.h)
					var camX=this.context.draw.cam.x;
					var camY=this.context.draw.cam.y;
				...
				if(document.activeElement.id!=this.context.canvas.id){
					...
					//change font size
					for(var i=0;i<9;i++){
						if(this.context.draw.ctx.measureText(string).width>this.context.canvas.w){
							this.context.draw.font("Consolas",20-(i*2));
						}else{
							break;
						}
					}
					...
					this.context.draw.alpha(alpha);
					this.context.draw.text(string,this.context.canvas.w/2,this.context.canvas.h/2-10,"white","center");
    ...
    this.draw={
        ...
        camReset:function(){
			this.cam.x=0;
			this.cam.y=0;
			this.cam.w=this.canvas.w;
			this.cam.h=this.canvas.h;
        },
		...
		xCam:function(x){
			var camW=Math.abs(this.canvas.w/this.cam.w)
			return (x/camW)+this.cam.x;
		},
				
		//camera to non camera vertical
		yCam:function(y){
			var camH=Math.abs(this.canvas.h/this.cam.h)
			return (y/camH)+this.cam.y;
		},
		
		//non camera to camera horizontal
		camX:function(x){
			var camW=Math.abs(this.canvas.w/this.cam.w)
			return (x-this.cam.x)*camW;
		},
				
		//non camera camera vertical
		camY:function(y){
			var camH=Math.abs(this.canvas.h/this.cam.h)
			return (y-this.cam.y)*camH;
		},
        ...
        fill:function(type,x,y,w,h,rotation,string){
            ...
            switch(type){
                ...
                case "arc":
                    ...
                    break;
				case "ellipse":
                    this.ctx.ellipse((x*camW)-(camX*camW),(y*camH)-(camY*camH),w*camW,h*camH,0,0,2*Math.PI);
					break;
                case "arcB":
                    this.ctx.arc((x*camW)-(camX*camW),(y*camH)-(camY*camH),w*((camW+camH)/2),0,2*Math.PI);
                    break;
	...
	this.particles={
		...
		addParticle:function(x,y,w,h,frames,wRate,hRate,alpha,alphaRate,vX,vY,aX,aY,c,cRate,cMax,r,rRate,o,oRate,image,anim,id){
			var part={};
			part.x=x;
			if(x==undefined){part.x=this.draw.canvas.w/2}
			part.y=y;
			if(y==undefined){part.y=this.draw.canvas.h/2}
			part.w=w;
			if(w==undefined){part.w=this.draw.canvas.w/100}
			part.h=h;
			if(h==undefined){part.h=this.draw.canvas.h/100}
			...
			part.rotation=r;
			if(r==undefined){part.rotation=0}
			part.rRate=rRate;
		...
		drawParticle:function(i){
			...
			if(p.anim===undefined && p.image===undefined){
				if(p.o===undefined){
					if(p.h==0 && p.hRate==0){
						this.draw.circle(p.x-p.w/2,p.y-p.w/2,p.w,p.c);
					}else{
						this.draw.rect(p.x-p.w/2,p.y-p.h/2,p.w,p.h,p.c,p.rotation);
					}
				}else{
					if(p.h==0 && p.hRate==0){
						this.draw.circleB(p.x-p.w/2,p.y-p.w/2,p.w,p.c,p.o);
					}else{
						this.draw.rectB(p.x-p.w/2,p.y-p.h/2,p.w,p.h,p.c,p.rotation,p.o);
					}
				}
			}else if(p.anim!==undefined){
				this.draw.anim(p.anim,p.x-p.w/2,p.y-p.h/2,p.w,p.h,p.rotation);
			}else if(p.image!==undefined){
				this.draw.image(p.image,p.x-p.w/2,p.y-p.h/2,p.w,p.h,p.rotation);
			}
    ...
    this.resize=function(w,h,camReset){
        return this.canvas.resize(w,h,camReset);
    }
	...
	this.camReset=function(){
		return this.draw.camReset();
	}
	
	this.camDefault=function(){
		return this.draw.camReset();
	}
	...
	this.xCam=function(x){
		return this.draw.xCam(x);
	}
		
	this.yCam=function(y){
		return this.draw.yCam(y);
	}
	
	this.camX=function(x){
		return this.draw.camX(x);
	}
		
	this.camY=function(y){
		return this.draw.camY(y);
	}
	...
	this.point=function(x,y,color,rotation){
        return this.draw.rect(x,y,1,1,color,rotation);
    }
    ...
    this.circle=function(x,y,diameter,color){
        return this.draw.circle(x,y,diameter,color);
    }
    
    this.circleB=function(x,y,diameter,color,lineW){
        return this.draw.circleB(x,y,diameter,color,lineW);
    }
	
	this.ellipse=function(x,y,diameterX,diameterY,color,rotation){
        return this.draw.ellipse(x,y,diameterX,diameterY,color,rotation);
    }
    
    this.ellipseB=function(x,y,diameterX,diameterY,color,rotation,lineW){
        return this.draw.ellipseB(x,y,diameterX,diameterY,color,rotation,lineW);
    }
    ...
    this.fontName=function(name){
		if(name!=undefined){
			this.draw.font(name,this.draw.fontSize)
		}
	...
	this.animPlay=function(name){
		return this.draw.animPlay(name)
	}
		
	this.animPause=function(name){
		return this.draw.animPause(name)
	}
	
	this.animStop=function(name){
		return this.draw.animStop(name)
	}
			
	this.animStopPlay=function(name){
		this.draw.animStop(name)
		return this.draw.animPlay(name)
	}
    ...
    this.color=function(col,type){
        return this.draw.color(col,type)
    }
	
	this.scale=function(scaleX,scaleY,x,y,w,h){
        return this.draw.scale(scaleX,scaleY,x,y,w,h);
    }
    ...
    this.linear=function(x1,y1,x2,y2,colors,offsets){
        return this.draw.linear(x1,y1,x2,y2,colors,offsets)
    }
    
    this.radial=function(x1,y1,d1,x2,y2,d2,colors,offsets){
        return this.draw.radial(x1,y1,d1,x2,y2,d2,colors,offsets)
    }
	...
	this.ceil=function(num){
		return this.math.ceil(num);
	}
	
	this.ceiling=function(num){
		return this.math.ceil(num);
	}
	...
	this.p=function(percent,start,end){
		return this.draw.percent(percent,start,end)
	}
    ...
    this.pX=function(percent,start,end){
        return this.draw.percentX(percent,start,end);
    }
    ...
    this.percentY=function(percent,start,end){
        return this.draw.percentY(percent,start,end);
    }
